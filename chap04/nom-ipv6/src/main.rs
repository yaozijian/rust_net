...
use nom::IResult;
use nom::{named,do_parse,bits,take_bits,map,take};
...
fn parse_ipv6_header(i : &[u8]) -> IResult<&[u8],IPv6Header>{
    ipparse(i)
}
...
fn main(){
    ...
    assert_eq!(parse_ipv6_header(&bytes),IResult::Ok((EMPTY_SLICE,expected)));
}
